...
public struct PostCard: Codable, Sendable, Equatable {
  ...
  /// D349 · **"has the golfer started?" is not "is `date` nil?"** The composer
  /// stamps today into `date` the moment it is constructed, so `isBlank` — which
  /// requires `date == nil` — is already false before anybody has touched
  /// anything. Every gate that means *the golfer has not started yet* must ask
  /// this instead, handing it the date the composer defaulted to: a `date` that
  /// still equals that default is the machine's answer, not the golfer's.
  ///
  /// `isBlank` keeps its own meaning (literally nothing, `date` included) and
  /// its own callers; this is the question the draft gates were asking wrong.
  public func isUntouched(defaultDate: String?) -> Bool {
    [whole, f9, b9, rating, slope, course].allSatisfy { $0.trimmingCharacters(in: .whitespaces).isEmpty }
      && !touched
      && (date == nil || date == defaultDate)
  }
  ...
  /// D354 · the plan's facts, and only those.
  ///
  /// The DATE is the fact that closes the loop: the suppression predicate
  /// matches a posted round to a plan on the day, so a round dated today
  /// neither closes the card nor scores in the right window. It is applied
  /// whatever the card says, because the card's date at this moment is the
  /// machine's stamp, not the golfer's choice — the caller has already
  /// established that.
  ///
  /// The COURSE is a convenience and goes in as TEXT, never as an id: without
  /// a tee there is no rating and no slope, and a stamped catalogue id beside
  /// typed figures would claim the catalogue supplied numbers it never
  /// supplied. This is exactly what `fill(memory:)` below does. A course the
  /// golfer has already named is left alone.
  ///
  /// Holes, gross, partners, photo and tee are never filled. A tag is not
  /// attendance, so the plan's tagged golfers do not become playing partners.
  public mutating func fill(plan: PlanContext) {
    if let d = plan.playOn, d.count == 10 { date = d }
    if let label = plan.courseLabel, !label.isEmpty,
       course.trimmingCharacters(in: .whitespaces).isEmpty {
      course = label
      courseId = nil
    }
  }
...
public struct PostDraft: Codable, Sendable, Equatable {
  ...
  /// D350 · the ordinary post's request identity, carried with the draft so a
  /// retry after the app is killed sends the SAME id. An id held only in memory
  /// is regenerated by the relaunch, which is precisely how a committed round
  /// becomes two. Optional because drafts written by older builds have none.
  public var request: UUID?
  /// D354 · the plan this draft is filling in, kept SEPARATE from `request`
  /// (the post's identity) and `sourceLive` (a kept scorecard's identity).
  /// Three different things that must never be confused for one another: a
  /// plan is display context, a request is what the server deduplicates on,
  /// and a kept scorecard is a card that already exists.
  public var plan: UUID?
  public init(at: Date = Date(), card: PostCard, sourceLive: UUID? = nil, request: UUID? = nil,
              plan: UUID? = nil) {
