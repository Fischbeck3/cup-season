  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 0) {
      Button { links.openReceipt(round.id) } label: { face }
        .buttonStyle(.plain)
        .accessibilityLabel("\(item.who.isEmpty ? "—" : item.who) — open the round")
  ...
  private var face: some View {
    HStack(alignment: hasPhoto ? .bottom : .center, spacing: 12) {
      ...
      VStack(alignment: .leading, spacing: 3) {
        ...
        Text(BoardLogic.courseLine(round)).font(CSFont.label).foregroundStyle(hasPhoto ? onPhotoMut : cs.mut).lineLimit(1)
