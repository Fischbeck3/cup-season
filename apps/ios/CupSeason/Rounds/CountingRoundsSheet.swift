...
struct CountingRoundsSheet: View {
  ...
  private struct Row: Identifiable {
    ...
    let points: Double?; let counting: Bool
  }
  private var rows: [Row] {
    (payload?["rounds"]?.array ?? []).map { r in
      Row(id: r["round_id"]?.string.flatMap(UUID.init), playedOn: r["played_on"]?.string ?? "",
          ...
          points: r["points"]?.double, counting: r["counting"]?.bool ?? true)
    }
  }
  ...
  private var rule: String { (cap ?? 0) > 0 ? "Best \(cap!) count each month" : "Every round counts" }

  var body: some View {
    NavigationStack {
      ScrollView {
        VStack(alignment: .leading, spacing: CSTokens.Space.s3) {
          Text("\(payload?["league_name"]?.string ?? door.leagueName ?? "") · \(when) · \(rule)")
          ...
          if let failed {
            ...
            VStack(spacing: 0) {
              ForEach(Array(rows.enumerated()), id: \.offset) { i, r in
                  ...
                  HStack(alignment: .firstTextBaseline, spacing: CSTokens.Space.s3) {
                    VStack(alignment: .leading, spacing: CSTokens.Space.s1) {
                      Text("\(r.gross.map(String.init) ?? "—") at \(r.course ?? "course not recorded")")
                        .csType(.bodyS).foregroundStyle(r.counting ? cs.ink : cs.mut)
                        .fixedSize(horizontal: false, vertical: true)
                      Text(r.playedOn + (r.holes == 9 ? " · 9 HOLES" : "")).csType(.agateS, caps: true).foregroundStyle(cs.mut)
                    }
                    .frame(maxWidth: .infinity, alignment: .leading)
                    Text((r.points.map(CSCopy.points) ?? "") + (r.counting ? "" : " · BUMPED"))
                      .csType(.columnS).foregroundStyle(r.counting ? cs.ink : cs.mut)
                ...
                .buttonStyle(.plain)
                .disabled(r.id == nil)
                .accessibilityIdentifier(r.counting ? "counting.round" : "counting.round.bumped")
              }
            }
            if rows.contains(where: { !$0.counting }) {
