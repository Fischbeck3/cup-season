...
### D273 · Money is ink and the ledger line is printed once per client
...
- **THE LEDGER LINE IS PRINTED ONCE PER CLIENT.** It renders verbatim from **one constant** — `MoneyCopy.ledger` on the phone, `CS_LEDGER` on the desk — in `agateS`, at the **foot of the money surface**, under a hairline. Two clients, two shapes, one printing each; that is D234's rule applied to a sentence rather than to a layout. It is not a caption, not a subtitle, not filler under a stat rail, and **it never appears on a surface whose subject is not money**: the Record page's career money and the ceremony's settlement card both dropped their copies in this commit. **A retyped copy is a defect the day it is written**, and the same law governs any future sentence of the same kind — a rules footnote, a disclosure, an eligibility clause: one surface, one printing, at the foot. If two lines is the cost, the constant is shortened **once, with the owner** — never on an individual surface.
