...
### D182 · The round that stops at eleven (PROPOSED 2026-08-31 · AMENDED + BUILT 2026-09-01)
...
- **AMENDED AND BUILT, 2026-09-01** (migration `20260901130000`). The shape holds; **one sentence in it was wrong, and building it as written would have been worse than the bug.**
  - **The engine correction.** "`score_round` **already** falls back to `p_rating/2`" is true of the **7-argument helper** — which is **not on the posting path**. The function that computes `rounds.differential` is the **0-argument trigger** (`rounds_before_insert`), and it had no such fallback: a nine with a null `nine_rating` fell through to the else-branch and subtracted the **full 18-hole rating**. Demonstrated on Postgres 16 with the real schema before writing the fix: a 46-stroke nine on a 71.5/130 course wrote **differential −22.2** — better than a tour pro's best round — and paid **6 points**. Relaxing the guard without also fixing the trigger would have corrupted the index of every golfer who walked off. The trigger now carries `coalesce(new.nine_rating, new.rating/2)`, which is the net under scan claims, old clients and any future caller.
  - **The scope correction.** `claim_round` carries the **identical** pair of guards. Fixing only `finish_live_round` would have moved the loss one step down the guest-claim funnel. Both are fixed in the same migration.
  - **What shipped:** rec 1 (a complete front nine posts as a nine whatever follows it; the dead `nine_rating` stop is removed and the value derived as half the 18 and **stored**, because the trigger reads the column) and rec 4 (the sheet names the holes that will not count). Extras are dropped by construction — every read is `v_strokes[1:v_holes]` — so there is no net par and no walking-off-while-hot exploit.
  - **Verified end to end on Postgres 16**, all four card shapes: 18 holes posts as 18 (unchanged); 14 holes posts the nine, drops 5; 10 holes posts the nine, drops 1, **differential 17.7 and index_at_post 11.4 — sane**; a gap in the front nine still posts nothing. A clean nine with no nine rating, which the old code skipped for "no 9-hole rating", now posts — which is why prod holds 1 nine-hole round in 211.
  - **Still open, deliberately:** rec 2 (a wizard/League door for `nine_hole_allowed`) is the owner's call and is **not** in this pass. Rec 3 (a `post_live_card` recovery RPC) is not built either, so a card **without** a complete front nine is still discarded — and the clients no longer claim otherwise. The web sheet now reads "a card that stopped after the turn posts as a nine — N holes past the ninth won't count toward it", and the false promise *"A partial card is skipped, not lost"* is gone. **The iOS client still carries that sentence** (`LiveCopy.swift`); native work is local-only per rule 6, so it is owed on the Mac.
  - **Not backfilled here.** The lost Raven Silver round (`a5429048`) needs its own one-off insert; it is a `status='final'` round, so the relaxed function will not pick it up.
