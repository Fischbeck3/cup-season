...
Open: coordinated database/Edge/schedule/web release and native distribution; real-device airplane-mode resume, guest/claim and OS share outcomes; scheduled cleanup with clients closed. The late-join participation-floor policy remains unchanged. See `docs/planning/2026-09-24-combined-audit-handoff.md` for current evidence and release order. Lane: Ops / launch · next action: authorize the coordinated release window after device checks.
...
### 2026-09-22 · The wizard's own run-back sentence still says "same crew"

`WizardState.runBackSub` reads *Same crew, same rules, fresh table — change anything in the wizard.* — the wizard door's sub, outside the four surfaces D375's packet named. The re-up asks everyone again and seats only the Pro, so "same crew" is the claim D375 retired everywhere else. Lane: UX · size: tiny · first question: does the wizard's run-back door survive at all now that `run_it_back` is the re-up, or is it the same door with the desk's sub?


### 2026-09-22 · The nine days to Oct 1, sequenced after the merge; the Mac session's prompt now lives in the repo

PR #6 and PR #7 merged this morning (`main` = `1e79279`; PR #5 closed as subsumed); preflight and the five unit files pass on the tip; the six migrations are recorded in production (252). The previous session wrote the execution prompt for the local Mac session outside the repository and it went with the sandbox, so it is rewritten from the tree as `docs/planning/2026-09-22-claude-october-launch-prompt.md`, beside the execution view `docs/planning/2026-09-22-october-launch-execution.md` (what is verifiably done, what is owed by day, the gates, the risks). Verified absent by grep, and owed on the Mac: every Swift half of D373–D378 (no caller of `Rpc.adjustPoints`, no unfinished-link face, no re-up copy), a build from the merged tip (934 is pre-merge), the two-phone checklist. Also owed: Codex's `20261111` then the `courses` redeploy, the review-notes re-point to a running season, the Support URL, counsel, the handles, the screenshots. Lane: Growth/Launch · size: the week · first question: does the first Owner build go up today from the web halves alone (device proof a day earlier, two archives) or wait for the Swift halves (one archive)? The plan assumes the former. *Revised the same day:* the year-end objective (5,000 first-time App Store downloads by Dec 31, checkpoints 500 / 2,000 / 5,000, assumptions unverified, TestFlight and web never counted), six workstreams with acceptance criteria, the owner's outreach kept apart, and the protocol corrected — a dedicated `claude/october-launch` branch, baseline then work, D376's exception confined, the latest Apple builds read, missing evidence vs confirmed absence, and `20261111` validated in the applied order (rename to sort after `20261115` only if the ledger says it is unapplied — `deploy-status` cannot see an applied version with no local file). *Refined the same evening:* photo consent governs every share output including the public page and the link preview on a reused token (`PostService.shareLink` uploads the copy unasked; W2's first half); a post-merge build and hardware results are missing evidence, not absence; the owner's weekly acquisition pipeline with a review by Oct 15; work continues while owner steps are pending.
