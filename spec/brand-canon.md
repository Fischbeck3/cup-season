...
dark green on warm paper. The one authoritative source is
`brand/candidates/testflight-pennant/source.json`, generated by
`tools/build-beta-mark.sh`; every raster in the web and iOS families comes from
it and none is hand-drawn or traced. The icon tile is a **fescue field, a cream
pennant and sparse, fine contours**, simplified at small sizes for legibility —
there is no solid ember identity tile. The mark may sign the welcome, the
mastheads, the app icon, boot and shared artifacts, **sparingly**; it is not a
