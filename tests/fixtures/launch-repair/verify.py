...
p = argparse.ArgumentParser(); p.add_argument('--socket', required=True); p.add_argument('--port', required=True); p.add_argument('--export-dir')
a = p.parse_args()
...
check('I4: every entry carries withdrawn=false while live', all(e.get('withdrawn') is False for r in after['rows'] for e in r['entries']) and after['version'] == 1 and after['frozen'] is False)
...
# Export the real combined RPC response for both client validators.
if a.export_dir:
    dest = pathlib.Path(a.export_dir)
    dest.mkdir(parents=True, exist_ok=True)
    for name, payload in [('audit-live', after), ('audit-withdrawn', gone)]:
        (dest / (name + '.json')).write_text(json.dumps(payload, separators=(',', ':')) + '\n')
...
# A legacy revocation with no public copies was never queued by the migration backfill.
LEGACY_OFF = f"insert into shares (token, kind, ref_id, created_by, revoked) values ('{A(71)}', 'round', '{R}', '{bid(2)}', true)"
old = steps(LEGACY_OFF, [f"to_jsonb(public.withdraw_round_shares('{R}'))", f"public.confirm_share_cleanup('{A(71)}')", stat], user=2)
check('I5 legacy withdrawal: every returned token can be confirmed even without old copies',
      A(71) in old[0] and old[1].get('status') == 'completed' and old[1].get('remaining') == 0 and not old[2]['cleanup_pending'], old)
