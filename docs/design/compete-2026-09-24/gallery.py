...
directions=['scoreboard','race','broadsheet']
fixtures=['field','tie','squads','upcoming','finished','multi']
surfaces=['root','season','entry','book','totals','race','week12','contributions','adjustments','receipt','round','root-bottom']
rows=sorted({r['key']:r for r in rows}.values(), key=lambda r:(directions.index(r['direction']),fixtures.index(r['fixture']),surfaces.index(r['surface']),r['phone'],r['size'],r['theme']))
...
<footer><p id="count"></p><a href="manifest.json">Capture manifest and image hashes</a></footer>
<script>
