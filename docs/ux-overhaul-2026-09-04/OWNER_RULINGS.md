...
> **AMENDED 2026-09-15 (D363).** R-F's *shape* stands — the golfer, then one more step, all three always offered, the state may only order them — but the *words* are replaced with the owner's authorisation: the head is **Play with ‹name›**, and the three are **Play a round** · **Go head to head** · **Start a season**. "This Saturday" read as a date restriction and "One week" as a duration nobody could verify. The person now rides into every route, and the head-to-head is reviewed (real closing date, basis, invitation state) before it is sent. See `spec/decision-log.md` D363.
