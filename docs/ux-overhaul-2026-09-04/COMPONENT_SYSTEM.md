...
| **AP-5** | **Emoji trophies** — 🎯 Broke 100, 🎯 Broke 90, 📉 Personal best (a falling chart for a best), 💪 Iron Man, ⚔️/🥊 for events | `TrophyMeta.swift:42-56`, `EventPickerSheet.swift:26-27` — SV-08 | **AMENDED BY D326 (2026-09-08); the original sentence is kept below it because the amendment is only legible against what it replaced.** ~~Emoji are the six reaction glyphs and nothing else~~ — D309 replaced those six with four **drawn** tokens, so that sentence lost its own subject and read as *no emoji anywhere*, which is not what it was reaching for. **AP-5 now binds SHARED marks only: a mark other people see is drawn; a mark only you see may be a glyph if it earns its place.** The reason is correctness, not taste — the fire is three different pictures across iOS, Android and the desk, so a shared glyph is not the glyph your league sees, while the post-round epilogue is one golfer, one moment, one platform. Achievements, trophies and empty-state icons on every shared surface stay **drawn strokes** in the marker family (`CSMarkerView`, `Marker.swift:15-40`; `SVGPath`, paths generated by `tools/build-markers.mjs`). Two achievements may never share a glyph. Enforced by `LINT-12`, whose own title and exemptions were corrected to match on 2026-09-09 | CL-4 |
