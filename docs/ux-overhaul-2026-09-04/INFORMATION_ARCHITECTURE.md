...
**The empty root — the state R-D named the tab for, written out.** Fourteen of thirty-nine prod profiles are in no league, and three of six personas land here with nothing running. The pattern is **P-11** and its door is required:
...
**Contacts matching is built here** (**R-G**, **C-11**): consent is asked at the point of the ask, in one sentence — *"We'll check your contacts against the golfers already here. We send hashes, never your contacts, and we keep nothing that doesn't match."* — declining still finishes the step, and **the empty result reads gracefully**: *"None of your contacts is here yet. Text one a link."* With 39 golfers on the app most matches return nothing, and the copy is written for that.
...
- **The name is asked last** and the `leagues` row is minted **at that tap, in one transaction with `lock_league`**, so an abandoned wizard leaves nothing behind. Prod holds six founder-alone `setup` leagues because today "Start the league" mints on a name (`WizardScreen.swift:70-86` → `:252-268`).
...
**Draft night, both seats.** Today `DraftNightScreen` shows the Pro's screen to a member with the verb swapped (CH-13), and branches three ways on `draft_type` while **all 13 prod leagues are `random`** — two of the three UIs have never been reachable.
...
- `assign` and `snake` are **specified as deleted** unless the wizard offers them. Carrying two unreachable UIs through a redesign is how the next audit finds them.
...
Until a meeting is confirmed, the facet is labelled *"played together"* and never *"beat"* without the qualifier. Same-day + same-course is the **fallback** heuristic and is labelled as such, because 155 of 212 quick rounds in prod carry no course id and the facet would otherwise be thin for months.
...
- **A gracefully empty result.** With 39 golfers on the app most matches return nothing: *"None of your contacts is here yet. Text one a link."* The feature is built for the shape of the product, not for this week's yield, and the copy says so by not promising.
...
Four frames, one of them optional. **No explainer slides and no orientation screen** (O-03, D224 — prod: `orientation_shown` 5, `orientation_done` **0**).
...
| `ClubRoute.album(UUID)` | `CompeteRoute.season(UUID, pane: .album)` | 1 photo exists in prod; it does not earn a segment |
...
- "Captains draft squads" is deleted — all 13 prod leagues draw at random and no captain drafts anything.
...
**Everything is behind one gate.** `device_tokens` holds one `ios-sandbox` row, `push_prompt_shown` fired once with zero accepts, and `invites` has zero rows ever. **No notification work starts until one production APNs token has received one real push.** This is a gate, not a risk to manage.
...
| **Two or more seasons** | §4.5 state L: no switcher, four items max, each naming its own season. `native_home`'s per-membership loop at four leagues is **unmeasured** — Wave 1b measures it before the deck ships. **Kept as a rule, not as a headline cost**: prod holds one real multi-league golfer and the "11 of 17" figure was a seeding artifact, so this does not drive the IA | `Models.swift:296-323` |
...
2. **`assign` and `snake`.** Delete both unreachable draft branches, or promote one deliberately in the wizard. All 13 prod leagues are `random`; carrying two unreachable UIs through a redesign is how the next audit finds them.
...
| **2** | **`round_players`' same-day/same-course fallback is a heuristic and is labelled as one.** 155 of 212 quick rounds in prod carry no course id, so R4's "played together" facet is thin for months until confirmations accumulate. | The alternative is inventing meetings, which L-44 forbids. The label is the honest fix. | Measurement after Wave 5: if confirmations stay under a third of tagged rounds, the facet is worth re-cutting rather than re-labelling. |
...
| **5** | **The event pot has no collected figure and no per-player paid state.** §8.1 therefore prints stake × field and no split on a Ryder. | Building `event_buy_ins` is a second money ledger for an object that lasts a weekend; §15.4 declines it in writing. | An organiser actually collecting money for a Ryder, which has never happened in prod. |
