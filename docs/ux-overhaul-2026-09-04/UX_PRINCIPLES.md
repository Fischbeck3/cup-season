...
- **How we check.** A cold-account walk: install → door → card → composer → posted, counting taps and elapsed seconds, measured against **the same walk on the shipped build** rather than against a production timing (`post_submit`'s n=24 is us, not a cohort, and is not a benchmark — `EVIDENCE_POLICY.md`). The number must go down.
...
Nine things the audit and the persona walks proved, each now a rule with a check. *Where a lesson below was originally evidenced by a count read from production, that read was a snapshot of an unlaunched database — scaffolding, not behaviour; see `EVIDENCE_POLICY.md`. The reads happened and are not erased; the rules stand on the brief, the canon and what the code does.*
...
| **A-2** | A golfer with no season — brand-new, between seasons, or with buddies and nobody organising — has no rung below the league: no story home, no push, no reaction, no invite. The brief requires that golfer have something to open the app for (P-1, P-5); the prod cohort ratio first offered here is void as behaviour (`EVIDENCE_POLICY.md`). **⚠ RE-ARGUE:** the rule stands on the brief; what does not is the **ranking** — that this is the biggest group and therefore the first rail work — which rested on the ratio alone and needs a walked leagueless seed or an escalation to rank again | **Every rail works without a season.** A round, a milestone, a reaction, a push and an invite must each function for a golfer in no league | Four probes on a leagueless seeded account: post a round → it has a story home; a buddy reacts; a buddy gets a push; a person link mints a buddy request |
| **A-3** | The wizard mints a `leagues` row the moment a name is typed — before the golfer has said what they want, and whether or not they ever finish. That is what the code does; the prod `setup`-league count first offered here is void as behaviour (`EVIDENCE_POLICY.md`) | **Nothing is minted until the golfer has said what they want.** (P-4) | The abandoned-wizard query |
...
| **A-8** | A screen whose only job is to teach the tabs asks a golfer to understand Cup Season before wanting it — the brief's one sentence forbids exactly that — and it teaches a nav **R-A** replaces while duplicating the doors beneath it. The prod `orientation_*` counts first offered here are void as behaviour (`EVIDENCE_POLICY.md`) | **No screen teaches the tabs.** What must be taught is taught where it is needed | The orientation file is deleted |
...
- **This forbids** an empty state that names **the golfer's absence**. *"Nothing posted this month"* opens on the golfer's own failure to post; *"Two rounds gets you back in the Fellas table before it closes"* opens on the move. The second is the only legal shape.
  - **Amended 2026-09-05 (F-7), and the amendment is the narrower reading, not a licence.** The Prove phase found the rule applied four times against the build's own producers — *"Nothing running."* (Compete), *"No buddies yet."* (Golfers), *"Nobody on the board yet."*, *"Nothing between you yet."* (head-to-head) — each with a legal sub and a live door beneath it. The forbid is about **whose absence is named**: an empty state may state a **fact about the world** ("there is no record between you yet") as its head, because that is not the golfer's failure and pretending otherwise makes the screen coy. It may **never** state the golfer's own omission as its head, and it must **always** end in a move. So the four producers stand as written; *"Nothing posted this month"* is still illegal.
  - **How to tell them apart in one question:** could the golfer have prevented this sentence by doing something? If yes, it is their absence and it may not lead. If no — nobody has played you yet, nobody is on the board yet — it is the world's, and it may.
...
| **2** | **§9's whole rule is gated on a channel that has never delivered.** One `ios-sandbox` token and no production token — the channel's own state, not a reading of the ask (nobody has been asked on a shipped build; `EVIDENCE_POLICY.md`). | It is a gate rather than a risk: no notification work begins until one production token receives one real push. Everything in §9 is a specification until then. | The Wave-0 gate passing. |
| **3** | **§2's five vision principles have no measurement yet.** Activation, time-to-aha and competition creation are named as success metrics with **no behavioural baseline at all** — `post_submit`'s n=24 is us, not a cohort, and is not one (`EVIDENCE_POLICY.md`). **⚠ RE-ARGUE:** the three metrics as the overhaul's yardstick rested on that baseline alone; to stand they need a launch, or restatement as the walks and escalations this set already runs. | Wave 0 stamps `app_open` / `home_state_seen` / `cta_tapped` / `first_act` and `platform` centrally; until then every target in this set is a target and says so. | The first month of those events. |
