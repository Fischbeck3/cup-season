...
| 30–33 | added by the Repair phase, not by this document: `Tour Card` · `the pool` · `playing number` outside the receipt · `start/join a league`. Each is a ruling that had drifted back with no lint behind it (D254, D255) | this table, §2.3, §3.2 | the receipt, where "playing number" is DEFINED beside its arithmetic |
