...
**Refined the same evening**, four points: production's migration ledger is
read before `20261111` is renamed, and an applied migration is preserved as
applied; photo consent governs every share output — the exported image, the
public share page and the link preview — including on a reused token; two
baseline claims that the evidence did not support are corrected (a morning
merge proves nothing about newer builds; hardware testing is missing evidence
until results are located or the checklist is run); the owner runs a weekly
acquisition pipeline, reviewed by October 15, and implementation keeps moving
while owner actions or production approvals are pending.
...
forecast. **The owner reviews the acquisition pipeline (§4A) against the
October 31 checkpoint by October 15**; if the pace is short, that review
decides the channel, not the target.
...
| **Database** | 252 migrations on disk, latest `20261115090000`. Production recorded the six pushed on 2026-09-22 (`20261109`, `20261110`, `20261112`–`20261115`), read back clean that hour. Codex's `20261111090000_course_cache_atomic` is **not in this tree**. **Whether production already carries it is missing evidence**: `deploy-status` subtracts the ledger from the local files, so a version applied remotely with no local file reads as *clean* — a clean summary does not answer the question. The Mac reads the ledger first (§2, Tue 22); it sorts *before* four applied migrations, so its ordering is a validation item either way. | Confirmed for the tree; production's six from the rulings packet §5 and the ACTIVE_WORK addendum, both written from the read-back; `20261111`'s state — missing evidence; `deploy-status` reads *unknown* here (no CLI) |
...
| **TestFlight** | The **record** names Owner group build 934 from `1aac23a` (pre-merge, uploaded 09-16) and Friends 795 (09-12). Whether any build was uploaded since — including one from the merged tip; this morning's merge proves nothing about what was archived after it — and what each group holds today, is **not known from the record**: the Mac reads the latest builds and both groups from App Store Connect before anything is archived. **The two-phone checklist**: no results are in the tree, and the record says NOT RUN as of 09-16; whether it has been run since is **missing evidence** until results are located or the checklist is run — never "never run". | Missing evidence, all three: the groups, a post-merge build, hardware results |
...
| **Measurement** | `v_pilot_gates` (D33) times the card and the join passively; post and standings have no passive timer. `v_growth_funnel` has no reader. The scorecard reads cohort, assistance, live games, posting, repeat groups, integrity; nothing reads App Store Connect; no acquisition pipeline exists anywhere (prospects, organizers, groups, downloads by channel). | Confirmed — migrations and `tests/pilot/scorecard.sql` |
...
| Tue 22 | Claude (Mac) + Owner | **`20261111`: the ledger first, then the ordering.** (0) **Read production's migration ledger** before anything else: `supabase migration list --linked`, the Remote column (read-only; `select version from supabase_migrations.schema_migrations order by version desc limit 12` says the same). `deploy-status` cannot answer this — it subtracts the ledger from the local files, so a version applied in production with no local file reads as *clean*. **If `20261111090000` is in the ledger it is applied: keep its name and its body exactly as applied, place the file in the tree, do not rename it** (rule 2), and record that the chain order differs from the applied order — a fact to note, not to repair. Only if it is absent from the ledger does the rest of this row apply. It sorts before four migrations production already carries, so file order and applied order differ. (1) Read its body: if it patches any function that `20261112`–`20261115` also patched in place, it must be rebased on the live text. (2) Because it has run nowhere but a sandbox, **rename it to sort after `20261115`** (`20261116090000_course_cache_atomic.sql`) so the file order equals the applied order — rule 2 forbids editing a migration that has run in production, and this one has not; expect the CLI to refuse or to demand its include-all flag if the old name is kept. (3) Sandbox chain: the 252, then the renamed file, the course-cache probes, an idempotent second run. (4) Owner: `supabase db push` → `supabase functions deploy courses` (with the `flattenTees` coercion in the function first) → `tests/db-checks.sql` 37/37 → `deploy-status` clean. | Production at 253; `courses` redeployed after it; db-checks green. |
...
| **Fri 25 – Sun 27** | Claude (Mac) | **W2 — the one round-sharing action** (§3), web half and phone half, after the owner's ruling on its shape (Thu 24, talk-first); the photo-consent half first, because it changes `PostService.shareLink` and `csShareLink` whatever shape the action takes. Fixes for anything the checklist or Friends surfaced, narrowly. The CSP header flipped to enforcing ONLY if the Netlify deploy console shows a clean report; otherwise file what fired and leave it. | Committed; suites green. |
...
| **Tue 29** | Owner + Claude | **W6 — the first weekly report** (§3): `node tools/pilot-scorecard.mjs --as-of 2026-09-27 --store-live false > docs/pilot/scorecard-2026-09-27.md` (dated the Sunday the week ended; W6 correction pass), the week's sessions entered before the numbers are read, the integrity section first; the five sections present. Ratify the amended gates (ruling 12). **Go / no-go for the submission, one written line.** Physical iPhone Safari pass of the public web door: the signed-out door, a claim link, an invite link, post a round. | Written. |
...
| **W2** | **One coherent round-sharing action**, respecting photo and privacy choices | Claude (web and phone halves); the owner rules on the shape Thu 24 | Today a posted round shares through four doors on the web (`shareRoundCard`, `shareRecapCard`, `shareSettlementCard`, `shareMajorCard`) plus the share link (`share_info`, D57), and the phone has `ShareIntent` / `ShareLinkService`. One **Share** action from a posted round, on both clients, that produces the round card and the share link together; the recap, settlement and major cards stay their own artifacts on their own surfaces. The photo rides the card only when the golfer attached it to that round and says yes on the share sheet; never another golfer's photo; the marker medallion stays (D59). The card never carries a golfer whose findability is *nobody* beyond their own share. Logged through `log_growth_event`. **Photo consent governs every output**, not only the card: the exported image, the public share page (`/?share=TOKEN` draws `shared/{token}.jpg` as the card's ground when `share_info.photo` is true) and the link preview (`netlify/edge-functions/share-preview.ts` sets `og:image` to the same copy). Today none of them ask: `PostService.shareLink` reads `rounds.photo_path` and uploads the copy to `shared/{token}.jpg` on its own; the web's `csShareLink` uploads it when the listing finds none; `create_share` re-returns the live token, and `share_info.photo` is `exists(copy)` — so a copy uploaded once serves the page and the preview for as long as the token lives. The fix: the yes or no is asked on the share sheet and carried into the mint on both clients; **no** means no upload, and on a reused token that already has a copy, the copy is deleted (the storage policy lets the sharer delete their own token's copy) or the token is revoked and re-minted so a cached preview cannot keep serving it; with the copy gone, `share_info.photo` reads false, the page draws no ground and the preview falls back to the brand image. | From a posted round on the web and on the phone, one action, the same artifact and the same sentence (one producer per client, D297); a round without a photo shares the card without one; **declining the photo keeps it out of all three outputs — the exported image, the public page, the preview — including when the token already existed with a copy** (verified by reading `share_info` for that token and fetching the copy's URL: false and 404); accepting puts it in all three; a Kit test on `shareLink` with consent false and a copy present; a web pin on the consent branch; the share appears in `v_growth_funnel`; a decision-log entry (IA level) before the build. | Consent half web and phone by Fri 27; the action's web half by Fri 27; its phone half on the Mon 28 build |
...
| **W6** | **Cohort, assistance, activation, sharing and acquisition reporting** | Claude extends the scorecard; owner runs it every Tuesday and reads App Store Connect | One weekly report, `tools/pilot-scorecard.mjs`, with five sections and a denominator on every number: **cohort** (owner · friends · independent · competition · founding, from `pilot_cohort_members`); **assistance** (assisted vs unassisted completions, from `pilot_sessions`); **activation** (account → first posted round → second round; groups with a second game); **sharing** (share links minted and opened, claims, invitations accepted — `v_growth_funnel` gets its first reader); **acquisition** (cumulative first-time App Store downloads from App Analytics against the checkpoints in §0, **by channel** where App Analytics' sources and the owner's pipeline (§4A) can attribute them; TestFlight installs and web sign-ups shown separately and never summed in; the pipeline's four counts — prospects contacted, organizers activated, groups playing, downloads by channel — carried in as the owner reports them). The App Store number is read by hand from App Store Connect until an `asc.py` command reads it. | The Tue 29 report has all five sections; each later Tuesday's report is written before the review; the acquisition line reads zero until approval and says so; the pipeline's four counts appear from the first week the owner records them; no report sums TestFlight or web into the store count. | First report Tue 29; the acquisition section live on approval day; the pipeline counts from the week of Oct 1 |
...
| Every Tuesday | The week's download count read from App Analytics into the log (a subtotal), and a dated cumulative reading recorded once a checkpoint date has passed — only a reading decides the checkpoint in §0; the week's line — hold · widen · stop. | The checkpoints steer; if Oct 31 misses 500 the review decides the channel, not the target. |

### 4A · The weekly acquisition pipeline — owner-run

One row a week, kept by the owner in a private sheet (names never enter the
repository), the four totals carried into the W6 report's acquisition section
every Tuesday. It is the owner's, not engineering's: nothing in it is built, and
no engineering item waits on it.

| Column | What is counted | Denominator |
|---|---|---|
| **Prospects contacted** | Organizers and groups the owner reached that week, by channel: by hand (independent groups), the public link, r/golf, PIGL's moments in the founder's voice, the web door | — |
| **Organizers activated** | Prospects who created or joined a season, a Ryder or a Major that week | prospects contacted to date |
| **Groups playing** | Groups that finished a first game unassisted that week (`pilot_sessions` says whether the founder was in the room) | organizers activated to date |
| **First-time downloads, by channel** | App Analytics' first-time downloads that week, attributed to a channel where the source data or the owner's own record allows; the unattributed remainder shown as such; TestFlight installs and web sign-ups in their own columns, never summed in | the §0 checkpoint |

**Review by October 15.** The owner and Claude read the pipeline's first two
weeks against the October 31 checkpoint of 500: the contact-to-activation and
activation-to-playing rates as measured, the downloads by channel, and the
projection they give. That review decides the channel mix for the rest of
October; the checkpoint itself does not move. Every assumption in §0 is
re-labelled that day — verified, refuted, or still unverified.
...
- **Device proof.** No hardware results are on record: the checklist reads NOT
  RUN as of 09-16 and nothing in the tree says otherwise. Everything after
  Wednesday depends on it, which is why the first Owner build goes up today from
  the web halves alone. The server side is already proven (72 restricted-role
  probes, row security on).
...
- **`20261111` may already be applied.** It lives on one laptop and Codex may
  have pushed it; `deploy-status` cannot see an applied version with no local
  file. Read the ledger first; an applied migration is preserved exactly as
  applied. Only an unapplied one is renamed, validated on the chain in the
  applied order, and never pushed under a name that sorts before what
  production already has.
- **A share can leak a photo the golfer declined.** Today the copy is uploaded
  without asking and a reused token keeps serving it to the page and the
  preview. W2's consent half closes that on both clients before the action
  ships.
...
7. **`20261111`: the ledger, then the order.** Production's migration ledger
   is read before any rename; an applied migration is preserved exactly as
   applied; an unapplied one is validated in the applied order before it is
   pushed (§2, Tue 22).
8. **Implementation keeps moving while owner actions or production approvals
   are pending.** A hand-over to the owner — a push, an upload, a group add, a
   submission, a ledger read, a ruling — is not a wait: the session takes the
   next item in §2 or §3 that does not depend on it and returns when the
   result is read back. It never performs the owner's step itself to unblock.
9. **Every session ends** with the handoff block the prompt specifies, the
