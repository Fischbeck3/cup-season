...
| **Database (linked project)** | **244 migrations applied**, latest `20261105090000_the_round_remembers_its_plan` (applied 2026-09-15 under written authorization; verified read-only afterwards). **Pending, written and validated on the full-chain sandbox, NOT applied:** `20261106090000_the_pilot_keeps_its_own_record` (pilot cohorts, sessions, retry-safe events) and `20261107090000_a_round_reads_its_own_holes` (owner may read own holes; `round_tally` says unknown without evidence). | `tools/deploy-status.mjs`; `tests/pilot/*.py` |
| **Edge functions** | 6 deployed, none stale, nothing pending. | `tools/deploy-status.mjs` |
| **Implemented but not on any tester's phone** | everything after `d93b6f4` on this branch: pilot telemetry (attempt-keyed start/join/finish events, receipt and clash exposure), the two pending migrations, the harness and materials. | this branch |
