...
The migrations; then `supabase functions deploy share-cleanup --no-verify-jwt`; `supabase secrets set SHARE_CLEANUP_SECRET=…`; and the required schedule in `tools/share-cleanup-schedule.sql`. Optionally add a Database Webhook on `public.share_cleanup` for **INSERT and UPDATE** that calls `share-cleanup` with header `x-cleanup-secret`. Verify the webhook's target from the database (CLAUDE.md) with the secret masked. A scheduled call is REQUIRED: configure the every-minute `cs-share-cleanup` job with `tools/share-cleanup-schedule.sql` after privately setting its two Vault values. A webhook cannot wake up at a future retry time or lease expiry. The worker reads only `_share_cleanup_due`; webhook payload tokens cannot bypass the backoff. A webhook is an optional additional wakeup, not the durability mechanism.
...
## Integration review corrections (Codex, local)

- Native removal now calls `confirm_share_cleanup` for each token and requires `completed` plus `remaining:0` before reporting success.
- Web receipt/epilogue controls retain failed withdrawals and show **Retry image cleanup** when bytes remain after the token is off. They withdraw every prior token, without minting one.
- `20261206090000_withdrawal_can_finish.sql` gives every token returned by withdrawal a confirmable cleanup row, including already-revoked legacy links without stored objects. Otherwise an old clean link would fail confirmation forever.
- Edge expiry failure returns HTTP 500; a broken expiry sweep is not reported as healthy.
- After configuring the schedule, prove an abandoned 15-minute preparation and a backed-off failure are reclaimed with both clients closed and no new webhook event. Confirm public JPG and PNG reads fail and the queue says completed. This production proof remains owed.
