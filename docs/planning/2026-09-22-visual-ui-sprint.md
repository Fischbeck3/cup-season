...
- Claude's next lane remains the [W6 reporting correction](2026-09-22-w6-review-and-claude-prompt.md). Avoid overlapping edits to `index.html`, shared design code and the native views during this sprint. A later cross-lane change must be integrated on an owned branch and rechecked.
...
This is the scope inventory, not a claim that every screen has been visually verified. **V1 has rendered evidence; remaining rows are queued for screen-by-screen capture.** Native fixtures give repeatable local evidence, but cannot prove a real invitation, push, production consent or two-device lifecycle.
...
| Home and navigation | `Home/`, `Main/`; `#view-home`, web tabs/sidebar | Empty, one/many seasons, before/after golf, live round, photo/no-photo, digest | V2; reuse HomeNoPhoto, HomePhotoStability and AfterGolfAnswer fixtures |
...
| Courses and schedule | `Courses/`, `Schedule/`; course sheets, `#view-schedule` | Search/loading/no result/failure, tees, saved/offline course, booking/date, long course names | V3; OfflineTripReview fixture available |
...
| Competition and season | `Compete/`, `Season/`, `League/`; `#view-compete`, `#view-hub`, standings | Empty/forming/live/finished, one/many competitions, Cup race, points path | V4; signed-in walkthrough required for routes not covered by fixtures |
| Events, draft and setup | `Events/`, `Draft/`, `Wizard/`; `#view-event`, `#view-draft`, `#view-wizard` | Major/Ryder/callout, forming/live/finished, member/Pro, validation, review/confirm | V4; LeagueSetupReview and event fixtures available |
...
**Current disposition:** V1's four corrections are implemented and locally verified; V2–V6 and the explicitly listed remaining entrance states are queued. Full visual readiness, acquisition outcomes, real recipient proof and release approval have not been claimed.
