...
**Integration follow-up:** Codex combined this branch with the native repairs and fixed cross-client defects. Use `2026-09-24-combined-audit-handoff.md` for the final candidate, current migration count, verification and release steps. The evidence below describes Claude’s original branch.
...
7. `20261124090000` S8 (D390; implementation authorized by the owner’s subsequent build instruction; see decision-log amendment)
...
- **`season_book` (originally version 2, corrected to version 1 by `20261205090000` for deployed readers):** entries gain `withdrawn` and `frozen`, and there is a `rules_note`.
...
1. Use the combined handoff: the owner’s build instruction already covered D390; release authorization is separate.
2. Agree a coordinated window with Codex's native branch, then merge.
3. `supabase db push` (19 migrations on the combined branch), then run `tests/db-checks.sql`.
4. `supabase functions deploy share-cleanup --no-verify-jwt`, then `supabase secrets set SHARE_CLEANUP_SECRET=…`, then configure the required every-minute schedule (`tools/share-cleanup-schedule.sql`); optionally create the Database Webhook that calls it with the `x-cleanup-secret` header. Verify the target with `pg_get_triggerdef`, masked, as CLAUDE.md requires.
