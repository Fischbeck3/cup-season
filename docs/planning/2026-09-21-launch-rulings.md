...
- **As built 2026-09-22** (owner: "build the season two re-ask now"): the re-up rides the invitation door, not the lock door — see the as-built note under D375 in `spec/decision-log.md`. `run_it_back` asks every living member (one invitation, one push each); a yes by `respond_invite` or `join_league` is recorded in `league_members.agreed_seasons`; the lens, the table, the hat, the start, the pot, the pulse and the record read it; the covenant and `my_invites` say the season; the Pro's roster counts the yeses and can ask again. The bylaws still carry locked (named in the note; an inbox item).
...
5. **Built 2026-09-21, remote:** `20261112090000_the_stage_d_cluster_before_the_door_opens.sql` (D378), `20261113090000_the_pros_pen.sql` (D376), `20261114090000_the_door_keeps_count.sql` (D371's throttle). Every function change is an in-place patch of the live definition (`pg_get_functiondef` → asserted replace → execute; the 20261102090000 pattern), idempotent on a second run. Validated on a Postgres 16 sandbox running PR #6's full 251-file chain (the harness's PG17 `MAINTAIN` privilege filtered from the stream; production is 17) with 18 functional probes. `tests/db-checks.sql` gains 35 (the pen reaches the table) and 36 (the bundle and the counter hold); check 1 now expects three cron jobs. Contract rows for `adjust_points`, `is_active_member`, `_door_gate`; `rpc.ts` and `Rpc.swift` regenerated. **Pushed to production by the owner 2026-09-22** (`supabase db push` from a worktree of this branch, together with `20261115090000` and PR #6's two, `20261109090000` and `20261110090000`; production's ledger read back with all six recorded). Verified read-only the same hour: every member carries a season on record (45 of 45), the lens and the individual table read it and keep security_invoker, the doors record the yes, the hat and the start read the season roster, the pot and the pen are in, `door_attempts` exists and no client role can read it, the anon surface still holds exactly the twelve, and pg_cron runs three jobs (`cs-daily-tick`, `cs-month-close`, `run_event_sessions`) with `cs-week-snapshot` gone. Codex review of the branch is still owed before the merge.
5b. **Built 2026-09-22, remote:** `20261115090000_season_two_is_a_re_up.sql` (D375, on the owner's "build the season two re-ask now"). `agreed_seasons` / `agreed_at` / `prior_left_at` on league_members with a BEFORE INSERT trigger and a backfill; `run_it_back` re-emitted to ask (invitations + pushes, the Pro's own yes, a board line saying the invitations are out); in-place patches (the 20261102090000 pattern, one temp helper with the anchors as data) to `respond_invite`, `join_league`, `randomize_squads`, `start_season`, `recompute_season_payouts`, `league_pulse`, `career_record`, `invite_golfer`, `home_dispatch`; `v_rounds_ranked` patched from its live definition (two conjuncts, security_invoker re-set); `v_individual_standings` re-emitted with the predicate; `my_invites` DROP + CREATE with `season_number` and `reup`; `join_covenant_info` re-emitted with `season_number`, `reup`, `agreed`, `last_season` and an agreed-only crew. Self-check counts the lens before and after (no round moved). Sandbox: full chain, 32 probes, idempotent second run (notices only). `tests/db-checks.sql` gains 37. Contract rows for `_agree_on_join`, `_agree_to_season`, `_season_roster` and the new `my_invites` shape; `rpc.ts` and `Rpc.swift` regenerated.
...
7. **Phone halves owed on the Mac (D234 — not done until both ship):**
   ...
   - D375 (built 2026-09-22, superseding the D375-now copy): `RunItBackResult` reads `asked` (falls back to `invited`, then to the bare "Season N is on.") and `line` says *N invitations are out — the table fills as they say yes* (`csRunItBackDone` twin, four cases pinned in `tests/app-tests.js`); `LeagueCopy.swift:551–566`'s *carried over* sentence retires. `MyInvite` decodes `season_number` and `reup`; the invitation card and the Home item title read *Season N invite* / *Season N of <league> is on. Same rules, fresh table.* (`csInviteTitle` twin). The covenant sheet (`JoinLeague.swift`) renders the `season` fact first (*Season 2. Last season you finished 3rd of 8 with 41 points.*), the re-up frame (`csCovenantTitle` / `csCovenantEyebrow` / `csCovenantButton` twins: *Season 2 of the Fellas* · *SAME RULES — EVERYTHING BEFORE YOU TAP* · *I'm in for season 2 — $50*) and stops on `agreed: true` with `csAlreadyInLine`. The accept toast says `csReUpDone` when the league was already in the golfer's memberships. `MembersSheet`: NOT IN YET beside a member whose `agreed_seasons` lacks the season, the eyebrow counts *N IN FOR SEASON 2*, and an **Ask again** row calls `Rpc.inviteGolfer` and toasts `csAskedAgain`. `Rpc.swift` is regenerated in this commit.
