STATUS: CONTRACT READY (v1 · 2026-09-25 · Claude, branch claude/social-course-backend-2026-09-26)
...
### `posted_round_thread(p_round uuid)` → jsonb
```json
{
  "ok": true,
  "round": {
  ...
  "count": 4,
  "comments": [ comment, … ]            // oldest first, max 200 (count is the true total)
}
```
...
### `my_notifications(p_before timestamptz default null, p_limit integer default 30)` → jsonb
```json
{
  "ok": true,
  "unread": 2,
  ...
  "next_before": "timestamptz|null"    // pass as p_before for the next page
}
```
`p_limit` clamped 1–50. Items whose comment was hidden/removed, whose round is no longer
...
`course_key` returns null for them). `p_tee` = a `tee_key` from `tees[]`; `p_holes` 9|18.
...
```json
{
  ...
  "selection": { "tee_key": "white@70.1/124", "tee_name": "White", "holes": 18 },
  "tees": [ { "key": "white@70.1/124", "name": "White", "gender": "male|female|null",
  ...
  "people": [
    {
      ...
      "rounds": [                                        // newest first, ≤30 per person, ALL tees/holes
        { "round_id": "uuid", "played_on": "2026-09-20", "gross": 72, "holes": 18,
          "tee_key": "white@70.1/124|null", "tee_name": "White|null",
...
`holes_played` equal to the selection. Gross only; no net, no points, no scoring change.
...
only when its `api_course_id` has cached `api_course_tees` rows with exactly its
`(rating, slope)` and the match is unambiguous: the tee named after the last " · " in
`course_label` (the picker's own format) among those candidates, else the single
distinct tee name among them. Anything else → `tee_key: null`, `tee_name: null`; the
round still shows in the golfer's history but never in a best.
`tee_key` = `lower(tee_name) || '@' || rating || '/' || slope` (rating as stored, e.g. `70.1`).
Nines: compared only with nines; front/back is not recorded and is not distinguished.
