...
### F9 — Applause glyph does not read as applause

**Owner follow-up, 2026-09-15:** the shipped-looking glyph is not recognizable as applause. This is a correction to F5's execution, not a reversal of the approved single action or applause/applauded vocabulary. `CSApplauseGlyph` uses a narrow, largely mirrored path with a central upright finger and sparks; at the displayed size the hands merge into an ambiguous symbol. Its selected fill is also translucent rather than the clearly filled silhouette in the approved direction.

Claude: redraw two recognizable overlapping hands with distinct palms/fingers and a slight diagonal; optional small motion marks only if legible. Keep the quiet icon/count treatment, muted outline and green selected state, current token stroke language and 44pt target. Match the web geometry. Inspect actual 22pt rendering as well as enlarged artwork, in both states and themes. Provide a phone-visible close-up plus in-feed capture; a new SVG path alone does not prove recognition. The current Claude worktree already has an uncommitted `ApplauseControl.swift` edit; coordinate within that owned worktree and preserve it. No change to appreciation writes or notification policy is requested here.

### F10 — Booked round: View round and Tee it up

**Owner instruction, 2026-09-15:** retire “Open the plan.” From a booked round, provide a quick “Tee it up” path into the live-round interface carrying the owner and other eligible golfers.

Use **View round** for the details door. In the booked-round details, make **Tee it up** the clear action into prepared live setup. Allow Home to expose the same action where appropriate, without turning a details tap into an undisclosed start/write. Preserve a final start confirmation for incomplete settings; if a linked live round already exists, offer **Join live round** or **Resume round** according to actual state/access. Do not automatically open another person's app.

Source: Home's action copy is server-produced in applied migrations, with `HomeFallbackItems` also carrying “Open the plan.” Correct the current producer through a new migration if necessary, never by editing historical migrations; update fallback copy and report database/client delivery separately. `ScheduledRoundSheet` has no direct prepared-live entry in the inspected source. `LiveRoundStore.loadPlan()` copies only the course label and matches `tagged_names` to roster names. Replace this incomplete bridge with identity-preserving handoff, reusing current authoritative plan/RSVP and course producers. The plan model already exposes `tagged_pids` and RSVP seats; inspect permissions and current live-round linkage before choosing implementation.
