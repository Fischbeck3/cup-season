...
| **LINT-18** | **probe** · ember discipline | more than **two** ember marks in one viewport, counting **every** `brand` fill, rule, glyph, dot and word **except the tab band** (a live dot plus its own eyebrow counts as one; a `CSCompetitionBand` is ONE mark however large it is). The fill-only version could not see the four screens whose only ember was the word `CLOSE`. **F11, 2026-09-15:** ember marks a COMPETITION in any state, not only a live one — the budget is unchanged, and the state is carried by a word beside the mark, never by the colour |
