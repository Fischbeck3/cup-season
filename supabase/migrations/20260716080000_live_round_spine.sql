-- ============================================================================
-- Cup Season — tee sheet arc, checkpoint 1: the live-round SPINE
--
-- "Score the round together on one phone; everyone's card posts at the end."
-- The tables already existed (live_rounds, live_round_players, round_holes);
-- this adds the two RPCs that make the loop real. Stroke play only — Match Play
-- (game_results + settlement) and Wolf ride on top in checkpoints 2 & 3.
--
--   start_live_round()  — persist the round + its players at tee-off. Guests get
--                         a claim_token (the recap/funnel seed). Returns the
--                         player-id map so the client can post cards at finish.
--   finish_live_round() — spec §13.1: every MEMBER's round posts at once, hole
--                         detail and all, ✓ attested by construction (co-play).
--                         The confirm gate is the finish itself; p_casual posts
--                         nothing (the closed-out / mulligan escape, batch-3 #14).
--                         Guests never post — they get a recap by claim_token.
--
-- member_id on live_round_players is league_members.id → resolve to profile_id
-- to post. Security-definer so one scorekeeper can post the group's cards
-- (rounds RLS is owner-only; co-play is the authorization, spec §13.1).
-- ============================================================================
...
declare
  v uuid := auth.uid();
  v_season uuid; v_lr uuid; v_pos int := 0; v_el jsonb;
begin
  if v is null then raise exception 'Sign in first'; end if;
  if not exists (select 1 from league_members where league_id = p_league and profile_id = v) then
...
begin
  ...
  -- participant check: the starter, or a member player whose profile is you
  if lr.started_by <> v and not exists (
  ...
  for v_card in select * from jsonb_array_elements(coalesce(p_cards, '[]'::jsonb)) loop
    ...
    -- guests: no post, carry their recap token
    ...
    -- parse the 18-slot card; count holes played (must be full 18 or full front-9)
    ...
    if p_casual then
      v_skipped := v_skipped || jsonb_build_object('name', playerlabel(v_pid), 'reason', 'casual');
      continue;
    end if;
    if v_holes is null then
      v_skipped := v_skipped || jsonb_build_object('name', playerlabel(v_pid), 'reason', 'incomplete card');
      continue;
    end if;
    if v_rating is null or v_slope is null then
      v_skipped := v_skipped || jsonb_build_object('name', playerlabel(v_pid), 'reason', 'no course rating');
      continue;
    end if;
    if v_idx is null then
      v_skipped := v_skipped || jsonb_build_object('name', playerlabel(v_pid), 'reason', 'no index set');
      continue;
    end if;
    if v_holes = 9 and v_nine is null then
      v_skipped := v_skipped || jsonb_build_object('name', playerlabel(v_pid), 'reason', 'no 9-hole rating');
      continue;
    end if;
...
-- tiny helper: a display label for a profile (used in the finish receipt)
create or replace function public.playerlabel(p_profile uuid)
returns text language sql stable security definer set search_path = public as $$
  select coalesce(display_name, 'A golfer') from profiles where id = p_profile;
$$;
